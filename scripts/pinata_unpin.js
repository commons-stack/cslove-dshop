 ...
 * $ ipfs pin ls | awk '{ print $1 }' | node pinata_unpin.js
 */
...
async function main(term) {
  ...
  console.log(`Pinata contains ${pins.length} pins.`, pinRes)

  for (const hash of pins) {
    console.log(`removing hash`, hash)
    try {
      const res = await unpin(hash)
      console.log(`hash removed`, res)
